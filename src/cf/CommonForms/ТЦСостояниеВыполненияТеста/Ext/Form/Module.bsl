...
Процедура ПриВыполненииТестирования()
	...
	Попытка
		ДатаНачала = ТекущаяДата();
		СостояниеТеста = ПолучитьПодробноеСостояниеТеста(мТест);
...
//
Функция ПолучитьПодробноеСостояниеТеста(Тест)
...
КонецФункции // ПолучитьПодробноеСостояниеТеста()
...
Процедура ОбновитьИндикаторы(СостояниеТеста, НомерСостояния, КоличествоВРМЗапущено, КоличествоВРМОсталосьЗапустить)
	...
	Если Состояние = Перечисления.ТЦСостояние.Подготовка
		ИЛИ Состояние = Перечисления.ТЦСостояние.Загрузка Тогда
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если Не мОстановлен И (ЗавершениеРаботы <> Неопределено И Не ЗавершениеРаботы) Тогда
