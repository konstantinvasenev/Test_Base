...
//   ИмяФайла - имя файла выгрузки
//
Функция ВыполнитьЗагрузку(Знач ИмяФайла, ТекстОшибки = "") Экспорт
	...
	Попытка
		...
		Если Не ЧтениеXML.Прочитать()
			...
			ТекстОшибки = Нстр("ru = 'Неверный формат файла выгрузки'");
			ПоказатьИЗаписатьОшибкуВЖурнал(ТекстОшибки);
			Возврат Ложь;
	...
	Исключение
		...
		ТекстОшибки = Нстр("ru = 'Неверный формат файла выгрузки " + Символы.ПС + ОписаниеОшибки() + "'");
		ПоказатьИЗаписатьОшибкуВЖурнал(ТекстОшибки);
		Возврат Ложь;
	КонецПопытки;
	...
	Пока Сериализатор.ВозможностьЧтенияXML(ЧтениеXML) Цикл
		...
		Исключение
			...
			Иначе
				...
				ПоказатьИЗаписатьОшибкуВЖурнал(ТекстСообщения);
...
Процедура ЗаписатьОшибкуВЖурнал(ТекстОшибки, Событие = Неопределено) Экспорт
	...
	Имя = ?(Событие = Неопределено, "Загрузка данных", Событие);
	ЗаписьЖурналаРегистрации(Имя, УровеньЖурналаРегистрации.Ошибка,, "Загрузка данных", ТекстОшибки);
	
КонецПроцедуры

Процедура ПоказатьИЗаписатьОшибкуВЖурнал(ТекстОшибки, Событие = Неопределено)
