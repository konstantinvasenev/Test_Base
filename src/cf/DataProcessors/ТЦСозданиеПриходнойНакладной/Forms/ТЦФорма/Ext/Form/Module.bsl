...
Функция ТЦИнициализировать() Экспорт
	...
	#Иначе
		...
		Показатель = ВыполнитьИнициализациюНаСервере();
		ТЦЗаписатьПоказатель("ВремяПроведенияПризходнойНаколаданой", Показатель);
...
&НаСервере
Функция ВыполнитьИнициализациюНаСервере()
	...
	Возврат ВремяОкончания - ВремяНачала - ТЦОбъект.Пауза * 1000; 
