...
//
Процедура ДобавитьПроизводительностьИтерацииВТест(Тест, ПараметрыИтерации);
	...
	Попытка
		...
		Запрос.УстановитьПараметр("НомерИтерации", ПараметрыИтерации.НомерТекущейИтерации);
		...
		НоваяСтрока.НомерИтерации = ПараметрыИтерации.НомерТекущейИтерации;
		НоваяСтрока.ДатаНачалаИтерации = ПараметрыИтерации.НачалоТекущегоСостоянияВыполнения;
		НоваяСтрока.ДатаОкончанияИтерации = ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения;
		НоваяСтрока.Длительность = ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения - ПараметрыИтерации.НачалоТекущегоСостоянияВыполнения;
		НоваяСтрока.КоличествоОпераций = ПараметрыИтерации.ТекущееКоличествоКО;
		НоваяСтрока.КоличествоВРМ = КоличествоВРМ;
		НоваяСтрока.APDEX = ПараметрыИтерации.ТекущаяПроизводительность;
...
Процедура УстановитьСостояниеТеста(Тест, Состояние)
	...
	Если Тест.Состояние <> Состояние Тогда
		...
		// Для состояния загрузки не меняем момент начала, т.к. загрузка "входит" в подготовку.
		Если НЕ (Тест.Состояние = Перечисления.ТЦСостояние.Подготовка И Состояние = Перечисления.ТЦСостояние.Загрузка) Тогда
...
Функция ДиагностироватьИЗавершитьТестСОшибкойФЗ(Тест, ИдЗадания = Неопределено) Экспорт
	...
	Попытка
		...
		Если Тест.Результат = Перечисления.ТЦРезультатВыполнения.Неопределено
			ИЛИ  Тест.Результат = Перечисления.ТЦРезультатВыполнения.ПустаяСсылка() Тогда
...
Функция ПолучитьКоличествоЗапущенныхИНезапущенных() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				КОГДА ТЦСценарийЗапуска.СостояниеПодготовки <> ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.НеГотовКЗапуску)
	               |						И ТЦСценарийЗапуска.СостояниеПодготовки <> ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.ГотовКЗапуску)
	               |					ТОГДА 1
	               |				ИНАЧЕ 0
...
Процедура ЗаданиеВыполнитьСценарий(Сценарий,
	...
	Попытка
		...
		ТЦОбщий.ЗаписатьВЖурнал(СокрЛП(Тест) + ": тест завершен успешно.", "Тест-центр");
	Исключение
		Ошибка = ИнформацияОбОшибке();
		ТЦОбщий.ЗаписатьВЖурнал(Ошибка, "Тест-центр");
		ТЦОбщий.ЗаписатьВЖурнал(СокрЛП(Тест) + ": тест завершен с ошибкой.", "Тест-центр");
...
Процедура ПриВыполненииСценария(Сценарий,
								Тест,
								ПараметрыВыполненияСценария)
								
	// Установим номера сеанса и соединения фонового задания
	НачатьТранзакцию();
	...
	Если Метаданные.РежимУправленияБлокировкойДанных = Метаданные.СвойстваОбъектов.РежимУправленияБлокировкойДанныхПоУмолчанию.Управляемый Тогда
		...
		ЭлементБлокировки = Блокировка.Добавить("Документ.ТЦТест");
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		ЭлементБлокировки.УстановитьЗначение("Ссылка", Тест);
		Блокировка.Заблокировать();
	КонецЕсли;
	
	ТестОбъект = Тест.ПолучитьОбъект();
	ТестОбъект.НомерСеансаФоновогоЗадания = НомерСеансаИнформационнойБазы();
	ТестОбъект.НомерСоединенияФоновогоЗадания = НомерСоединенияИнформационнойБазы();
	ТестОбъект.Записать();
	
	ЗафиксироватьТранзакцию();
								
	// Определим общее количество запускаемых ВРМ							
	Если ПараметрыВыполненияСценария.ОбщееКоличествоОдновременноЗапускаемыхВРМ = Неопределено
			ИЛИ ПараметрыВыполненияСценария.ОбщееКоличествоОдновременноЗапускаемыхВРМ < 0 Тогда
		ПараметрыВыполненияСценария.ОбщееКоличествоОдновременноЗапускаемыхВРМ = Сценарий.ОбщееКоличествоОдновременноЗапускаемыхВРМ;
	КонецЕсли;
	
	Если ПараметрыВыполненияСценария.ОбщееКоличествоОдновременноЗапускаемыхВРМ < 0 Тогда
		ПараметрыВыполненияСценария.ОбщееКоличествоОдновременноЗапускаемыхВРМ = ТЦОбщий.ОбщееКоличествоОдновременноЗапускаемыхВРМПоУмолчанию();
	КонецЕсли;
	
	// Определим общее количество запускаемых ВРМ для одного агента Тест-центра
	Если ПараметрыВыполненияСценария.КоличествоОдновременноЗапускаемыхВРМАгента = Неопределено
		ИЛИ ПараметрыВыполненияСценария.КоличествоОдновременноЗапускаемыхВРМАгента < 0 Тогда
		
		ПараметрыВыполненияСценария.КоличествоОдновременноЗапускаемыхВРМАгента = Сценарий.КоличествоОдновременноЗапускаемыхВРМАгента;
	КонецЕсли;
	
	Если ПараметрыВыполненияСценария.КоличествоОдновременноЗапускаемыхВРМАгента <= 0 Тогда
		ПараметрыВыполненияСценария.КоличествоОдновременноЗапускаемыхВРМАгента = ТЦОбщий.КоличествоОдновременноЗапускаемыхВРМАгентаПоУмолчанию();
	КонецЕсли;
	
	ЭтоДинамическийТест = ПараметрыВыполненияСценария.ЭтоДинамическийТест;
	ОбщееКоличествоОдновременноЗапускаемыхВРМ = ПараметрыВыполненияСценария.ОбщееКоличествоОдновременноЗапускаемыхВРМ;
	МаксимальноеКоличествоИтераций = ПараметрыВыполненияСценария.МаксимальноеКоличествоИтераций;
	
	ПараметрыИтерации = Новый Структура;
	
	ПараметрыИтерации.Вставить("НомерТекущейИтерации", 0);
	ПараметрыИтерации.Вставить("НужныДополнительныеИтерации", Ложь);
	ПараметрыИтерации.Вставить("ТекущаяПроизводительность", 0);
	ПараметрыИтерации.Вставить("ТекущееКоличествоКО", 0);
	ПараметрыИтерации.Вставить("ТекущийШагДозапускаПользователей", ПараметрыВыполненияСценария.НачальныйШагДозапускаПользователей);
	ПараметрыИтерации.Вставить("ТекущееКоличествоПользователей", ПараметрыВыполненияСценария.НачальноеКоличествоПользователей);
	ПараметрыИтерации.Вставить("КоличествоПользователейПредыдущейИтерации", 0);
	ПараметрыИтерации.Вставить("ОбщееКоличествоОшибок", 0);
	ПараметрыИтерации.Вставить("ТекущееКоличествоОшибок", 0);
	ПараметрыИтерации.Вставить("НачалоТекущегоСостоянияВыполнения", Неопределено);
	ПараметрыИтерации.Вставить("ОкончаниеТекущегоСостоянияВыполнения", Неопределено); 
	ПараметрыИтерации.Вставить("ПредыдущийСценарийЗапуска", Неопределено);
	ПараметрыИтерации.Вставить("СценарийПредыдущейИтерации", Неопределено);
	
	// Построим таблицу распределения по имеющимся агентам
	Если ПараметрыВыполненияСценария.РаспределятьПоКомпьютерамРавномерно Тогда
		...
		ПараметрыВыполненияСценария.ТаблицаРаспределения = Результат.Выгрузить();
	...
	ТаймаутОпросаАгентов = Сценарий.ТаймаутОпросаАгентов;
	Если ТаймаутОпросаАгентов < 1 Тогда
	...
	МаксимальноеКоличествоПользователей = ПараметрыВыполненияСценария.НачальноеКоличествоПользователей + ПараметрыВыполненияСценария.НачальныйШагДозапускаПользователей * (МаксимальноеКоличествоИтераций - 1);
	ИспользоватьТолькоКлонов = Сценарий.Структура.Итог("Количество") < МаксимальноеКоличествоПользователей;
	
	Если МаксимальноеКоличествоИтераций > 1 Тогда
		СтруктураМаксимальнойИтерации = ПолучитьМасштабированнуюСтруктуруСценария(Сценарий, Неопределено, МаксимальноеКоличествоПользователей, 0, "", Ложь, Неопределено); // Здесь масштабируем только на нужное количество пользователей
	Иначе // для единственной итерации рассчитаем структуру итерации полностью
		СтруктураМаксимальнойИтерации = ПолучитьМасштабированнуюСтруктуруСценария(Сценарий, Неопределено, ПараметрыИтерации.ТекущееКоличествоПользователей, 0, ПараметрыВыполненияСценария.АдресПубликации, ПараметрыВыполненияСценария.ЕстьРазделение, ПараметрыВыполненияСценария.ТаблицаРаспределения);
	КонецЕсли;
	
	НастроитьПользователей(СтруктураМаксимальнойИтерации, ИспользоватьТолькоКлонов);
	
	// Проверим, не прерван ли тест
	НачатьТранзакцию();
	Прерван = ТестПрерван(Тест);
	ЗафиксироватьТранзакцию();
	...
	ТаблицаВыполнения	= СформироватьТаблицуВыполнения(Сценарий, Тест, ПараметрыВыполненияСценария);
	...
	Для Каждого Задание Из ТаблицаВыполнения Цикл
		...
		Если ТекущееСостояние = Перечисления.ТЦСостояние.Подготовка
			И НЕ Задание.ЭтоПовтор Тогда
			
			ПараметрыИтерации.ТекущееКоличествоОшибок = 0;
		КонецЕсли;
		
		// Если текущее задание - подготовка, и номер итерации изменился,
		// следует выполнить подготовительные действия над сценарием
		Если ПараметрыИтерации.НомерТекущейИтерации <> Задание.НомерИтерации Тогда
			
			ПараметрыИтерации.НомерТекущейИтерации = Задание.НомерИтерации;
			...
			Пока СценарийРассчитан = Ложь Цикл
				...
				Если МаксимальноеКоличествоИтераций > 1 ИЛИ ЭтоПовторныйРасчет Тогда
					ФактическаяСтруктура = ПолучитьМасштабированнуюСтруктуруСценария(Сценарий, ПараметрыИтерации.СценарийПредыдущейИтерации, ПараметрыИтерации.ТекущееКоличествоПользователей, ПараметрыИтерации.КоличествоПользователейПредыдущейИтерации, ПараметрыВыполненияСценария.АдресПубликации, ПараметрыВыполненияСценария.ЕстьРазделение, ПараметрыВыполненияСценария.ТаблицаРаспределения);
				...
				ДобавитьСтруктуруИтерацииВТест(Тест, ПараметрыИтерации.НомерТекущейИтерации, ФактическаяСтруктура);
				
				КоличествоВРМПоКомпьютерам = ПолучитьКоличествоВРМПоКомпьютерам(ФактическаяСтруктура);
				...
				Если ТестПрерван(Тест) Тогда
					...
					Если ПараметрыИтерации.НачалоТекущегоСостоянияВыполнения <> Неопределено
						И ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения = Неопределено Тогда
						
						ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения = ТекущаяУниверсальнаяДата();
						ПолучитьУдовлетворительностьПроизводительности(ПараметрыВыполненияСценария, ПараметрыИтерации);
						ДобавитьПроизводительностьИтерацииВТест(Тест, ПараметрыИтерации);
				...
				Если КоличествоКомпьютеровБезАгентов > 0 Тогда
					...
					Для Каждого ТекущийКомпьютер Из КомпьютерыБезАгентов Цикл
						ВесОшибки = ВесОшибки + КоличествоВРМПоКомпьютерам[ТекущийКомпьютер];
					КонецЦикла;
					
					Если КоличествоВРМПоКомпьютерам.Количество() - КоличествоКомпьютеровБезАгентов = 0 Тогда
					...
					ОбработатьОшибку(БуферОшибок, Тест, Агенты, ПараметрыИтерации.НомерТекущейИтерации, ПараметрыИтерации.ТекущееКоличествоОшибок, Ошибка);
					
					Если ПараметрыВыполненияСценария.ТаблицаРаспределения <> Неопределено Тогда
						Сч = ПараметрыВыполненияСценария.ТаблицаРаспределения.Количество() - 1;
						Пока Сч >= 0 Цикл
							ТекущийКомпьютер = ПараметрыВыполненияСценария.ТаблицаРаспределения[Сч].Компьютер;
							Если КомпьютерыБезАгентов.Найти(ТекущийКомпьютер) <> Неопределено Тогда
								ПараметрыВыполненияСценария.ТаблицаРаспределения.Удалить(Сч);
							КонецЕсли;
							Сч = Сч - 1;
						КонецЦикла;
					КонецЕсли;
					
					Если ПараметрыИтерации.ПредыдущийСценарийЗапуска <> Неопределено Тогда
						Сч = ПараметрыИтерации.ПредыдущийСценарийЗапуска.Количество() - 1;
						Пока Сч >= 0 Цикл
							ТекущийКомпьютер = ПараметрыИтерации.ПредыдущийСценарийЗапуска[Сч].Компьютер;
							Если КомпьютерыБезАгентов.Найти(ТекущийКомпьютер) <> Неопределено Тогда
								ПараметрыИтерации.ПредыдущийСценарийЗапуска.Удалить(Сч);
							КонецЕсли;
							Сч = Сч - 1;
						КонецЦикла;
					КонецЕсли;
					
					// Отметим, что сценарий необходимо рассчитать повторно.
					ЭтоПовторныйРасчет = Истина;
					
				КонецЕсли;			
				
				// Подготовим сценарий запуска для расчета количества ВРМ, относящихся к каждому агенту.
				ПодготовитьСценарийЗапуска(ФактическаяСтруктура, ИспользоватьТолькоКлонов, ПараметрыИтерации.ПредыдущийСценарийЗапуска,
											ЭтоДинамическийТест, ОбщееКоличествоОдновременноЗапускаемыхВРМ);
				
				Агенты = АгентыСценария(ФактическаяСтруктура);
				КоличествоВРМАгентов = ПолучитьКоличествоВРМАгентов(Агенты);
				
				КоличествоАгентовПоКомпьютерам = Новый Соответствие;
				Для Каждого ТекущийАгент Из Агенты Цикл
					ТекущееКоличествоАгентов = КоличествоАгентовПоКомпьютерам[ТекущийАгент.Компьютер];
					Если ТекущееКоличествоАгентов = Неопределено Тогда
						КоличествоАгентовПоКомпьютерам.Вставить(ТекущийАгент.Компьютер, 1);
					Иначе
						КоличествоАгентовПоКомпьютерам.Вставить(ТекущийАгент.Компьютер, ТекущееКоличествоАгентов + 1);
					КонецЕсли;
				КонецЦикла;
				...
				// Обработаем возможное прерывание теста
				Если ТестПрерван(Тест) Тогда
					
					Если ПараметрыИтерации.НачалоТекущегоСостоянияВыполнения <> Неопределено И ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения = Неопределено Тогда
						ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения = ТекущаяУниверсальнаяДата();
						ПолучитьУдовлетворительностьПроизводительности(ПараметрыВыполненияСценария, ПараметрыИтерации);
						ДобавитьПроизводительностьИтерацииВТест(Тест, ПараметрыИтерации);
					КонецЕсли;
				...
				Если РезультатПроверки.КоличествоНеактивных > 0 Тогда
					...
					НеОтветившиеАгенты = РезультатПроверки.Неполучившие;
					ЭтоКритическаяОшибка = Ложь;
					ВесОшибки = 0;
					
					Если НеОтветившиеАгенты.Количество() > 0 Тогда
						Список = ТЦОбщий.МассивВСтроку(РезультатПроверки.Неполучившие, "
							| - ", Истина);
						ТекстОшибки = "Следующие агенты не ответили за таймаут опроса " + ТаймаутОпросаАгентов + " с (возможно, не запущены):" + Список;
						
						Для Каждого ТекущийНеответивший Из НеОтветившиеАгенты Цикл
							ВесОшибки = ВесОшибки + КоличествоВРМАгентов[ТекущийНеответивший] + 1;
						КонецЦикла;					
					КонецЕсли;
					...
					УдалитьНеОтветившихАгентов(Агенты, НеОтветившиеАгенты, КоличествоАгентовПоКомпьютерам, ПараметрыВыполненияСценария.ТаблицаРаспределения, ПараметрыИтерации.ПредыдущийСценарийЗапуска);
					ЭтоПовторныйРасчет = Истина;

				КонецЕсли;
				
				Если ЭтоПовторныйРасчет Тогда
					ПараметрыИтерации.СценарийПредыдущейИтерации = ФактическаяСтруктура;
					ПараметрыИтерации.КоличествоПользователейПредыдущейИтерации = ПараметрыИтерации.ТекущееКоличествоПользователей;
					Продолжить;
				КонецЕсли;
		...
		// Поменяем статус ВРМ в сценарии запуска для ВРМ со статусом "ОшибкаЗагрузки"
		Если ТекущееСостояние = Перечисления.ТЦСостояние.Подготовка И Задание.ЭтоПовтор Тогда
			
			Если ОбщееКоличествоОдновременноЗапускаемыхВРМ <> 0 Тогда
				НовыйСтатус = Перечисления.ТЦСостояниеПодготовкиВРМ.НеГотовКЗапуску;
			Иначе
				НовыйСтатус = Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗапуску;
			КонецЕсли;
			...
			Запрос.Текст = "ВЫБРАТЬ
			               ...
			               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска
			               |ГДЕ
			               |	ТЦСценарийЗапуска.СостояниеПодготовки = ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.ОшибкаЗагрузки)";
			
			Выборка = Запрос.Выполнить().Выбрать();
			Пока Выборка.Следующий() Цикл
				ОтметитьСостояниеПодготовкиВРМ(Выборка.Ид, НовыйСтатус);
			КонецЦикла;
		...
		Если ТекущееСостояние = Перечисления.ТЦСостояние.Выполнение Тогда
			...
			Если НЕ ЭтоДинамическийТест ИЛИ Не ПервыйЭтапВыполненияУжеБыл Тогда
				ПараметрыИтерации.НачалоТекущегоСостоянияВыполнения = ТекущаяУниверсальнаяДата();
		...
		СостоянияАгентов.Колонки.Добавить("КоличествоВРМ");
		...
		Для каждого Агент Из Агенты Цикл
			...
			Если КоличествоВРМАгентов <> Неопределено Тогда
				СостояниеАгента.КоличествоВРМ = КоличествоВРМАгентов[Агент];
			КонецЕсли;
		КонецЦикла;
		...
		Готово = Ложь;
		// На этапе загрузки ВРМ таймаут не обновляем, все происходит в рамках таймаута подготовки.
		Если ТекущееСостояние <> Перечисления.ТЦСостояние.Загрузка Тогда
		...
		// Ожидать отведенное на этап теста время
		ДобавленПовторПодготовкиИИлиЗагрузки = Ложь;
		Пока ТекущаяДата() < Таймаут Цикл
			
			Если ОбщееКоличествоОдновременноЗапускаемыхВРМ <> 0 Тогда
					
				// Отметим готовность ВРМ для запуска
				Если ТекущееСостояние = Перечисления.ТЦСостояние.Подготовка Тогда
					ОтметитьВРМКЗапуску(ОбщееКоличествоОдновременноЗапускаемыхВРМ);
					
				// Отметим готовность ВРМ для загрузки
				ИначеЕсли ТекущееСостояние = Перечисления.ТЦСостояние.Загрузка Тогда
					ОтметитьВРМКЗагрузке(ОбщееКоличествоОдновременноЗапускаемыхВРМ);
				...
				Иначе // Если есть сообщение
					...
					ИначеЕсли Сообщение.Сообщение = Перечисления.ТЦСообщение.ВыполненоСОшибкой Тогда
						...
						Если ТипЗнч(Сообщение.Параметры) = Тип("Структура") Тогда
							...
							Если СостояниеПриОшибке = Перечисления.ТЦСостояние.Подготовка И НЕ ДобавленПовторПодготовкиИИлиЗагрузки Тогда
								
								ТЦОбщий.ЗаписатьВЖурнал("Запланировано повторение подготовки");
								ДобавитьПовторнуюПодготовку(ПараметрыИтерации.НомерТекущейИтерации, ТаблицаВыполнения.Индекс(Задание) + 1, ТаблицаВыполнения, Сценарий, Тест, ПараметрыВыполненияСценария);
								ДобавленПовторПодготовкиИИлиЗагрузки = Истина;
								
							ИначеЕсли СостояниеПриОшибке = Перечисления.ТЦСостояние.Загрузка И НЕ ДобавленПовторПодготовкиИИлиЗагрузки Тогда
								
								ТЦОбщий.ЗаписатьВЖурнал("Запланировано повторение подготовки и загрузки");
								ДобавитьПовторнуюПодготовкуИЗагрузку(ПараметрыИтерации.НомерТекущейИтерации, ТаблицаВыполнения.Индекс(Задание) + 1, ТаблицаВыполнения, Сценарий, Тест, ПараметрыВыполненияСценария);
								ДобавленПовторПодготовкиИИлиЗагрузки = Истина;
			...
			Если Готово Тогда
				...
				Если ТекущееСостояние = Перечисления.ТЦСостояние.Выполнение	Тогда
					ОбработатьЗавершениеВыполнения(Сценарий, Тест, ПараметрыВыполненияСценария, ПараметрыИтерации, ТаблицаВыполнения);
				КонецЕсли;
			...
			Иначе // Если не все Агенты выполнили задание, пауза 3с и повтор проверки
				...
				Если ТестПрерван(Тест) Тогда
					...
					ИзменитьТест(Тест, "УстановитьСостояниеТеста", ПредопределенноеЗначение("Перечисление.ТЦСостояние.Неактивен"));
					ТЦОбщий.ЗаписатьВЖурнал(СокрЛП(Тест) + ": тест прерван.", "Тест-центр");
					Возврат;
		...
		Если Не Готово Тогда
			...
			// Учем таймауты агентов
			ЭтоКритическаяОшибка = Ложь;
			НеОтветившиеАгенты = Новый Массив;
			ВесОшибки = 0;
			
			Для каждого СостояниеАгента Из СостоянияАгентов Цикл
				
				Если СостояниеАгента.Выполнено = Ложь Тогда
					НеОтветившиеАгенты.Добавить(СостояниеАгента.Агент);
					ВесОшибки = ВесОшибки + КоличествоВРМАгентов[СостояниеАгента.Агент] + 1;
				КонецЕсли;
				
			КонецЦикла;
			
			ТекстОшибки = Вычислить(Задание.Ошибка + "(Сценарий, Тест)") + ТЦОбщий.МассивВСтроку(НеОтветившиеАгенты, "
				| - ", Истина);
			
			Если СостоянияАгентов.Количество() = НеОтветившиеАгенты.Количество() Тогда
				
				ТекстОшибки = "Получена критическая ошибка: не осталось ни одного активного агента." + Символы.ПС + ТекстОшибки;
				ЭтоКритическаяОшибка = Истина;
			...
			// Удалим неответивших агентов из теста
			УдалитьНеОтветившихАгентов(Агенты, НеОтветившиеАгенты, КоличествоАгентовПоКомпьютерам, ПараметрыВыполненияСценария.ТаблицаРаспределения, ПараметрыИтерации.ПредыдущийСценарийЗапуска);
			
			Если ТекущееСостояние = Перечисления.ТЦСостояние.Выполнение	Тогда // Проверим нужно ли продолжать выполнение теста, дополним таблицу выполнения, если нужно.
				
				ОбработатьЗавершениеВыполнения(Сценарий, Тест, ПараметрыВыполненияСценария, ПараметрыИтерации, ТаблицаВыполнения);
				
			ИначеЕсли ТекущееСостояние = Перечисления.ТЦСостояние.Подготовка Тогда
				
				Если НЕ ДобавленПовторПодготовкиИИлиЗагрузки Тогда
					ТЦОбщий.ЗаписатьВЖурнал("Запланировано повторение подготовки");
					ДобавитьПовторнуюПодготовку(ПараметрыИтерации.НомерТекущейИтерации, ТаблицаВыполнения.Индекс(Задание) + 1, ТаблицаВыполнения, Сценарий, Тест, ПараметрыВыполненияСценария);
				КонецЕсли;
...
Процедура ОбработатьЗавершениеВыполнения(Сценарий, Тест, ПараметрыВыполненияСценария, ПараметрыИтерации, ТаблицаВыполнения)
	
	ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения = ТекущаяУниверсальнаяДата();
	
	// Получим производительность
	ПроизводительностьУдовлетворительна = ПолучитьУдовлетворительностьПроизводительности(ПараметрыВыполненияСценария, ПараметрыИтерации);
	...
	Если ПараметрыИтерации.НомерТекущейИтерации >= ПараметрыВыполненияСценария.МаксимальноеКоличествоИтераций Тогда
		НужныДополнительныеИтерации = Ложь;
	Иначе
		
		Если НЕ ПроизводительностьУдовлетворительна И ПараметрыИтерации.ТекущийШагДозапускаПользователей <> ПараметрыИтерации.КонечныйШагДозапускаПользователей
			И ПараметрыИтерации.НомерТекущейИтерации <> 1 Тогда
			
			НужныДополнительныеИтерации = Истина;
			ПараметрыИтерации.ТекущийШагДозапускаПользователей = ПараметрыИтерации.КонечныйШагДозапускаПользователей;
			ПараметрыИтерации.ТекущееКоличествоПользователей = ПараметрыИтерации.ТекущееКоличествоПользователей - ПараметрыИтерации.НачальныйШагДозапускаПользователей + ПараметрыИтерации.ТекущийШагДозапускаПользователей;
			
		ИначеЕсли НЕ ПроизводительностьУдовлетворительна
			И (ПараметрыИтерации.ТекущийШагДозапускаПользователей = ПараметрыИтерации.КонечныйШагДозапускаПользователей ИЛИ ПараметрыИтерации.НомерТекущейИтерации = 1)Тогда
			
			НужныДополнительныеИтерации = Ложь;
			
		Иначе
			НужныДополнительныеИтерации = Истина;
			ПараметрыИтерации.ТекущееКоличествоПользователей = ПараметрыИтерации.ТекущееКоличествоПользователей + ПараметрыИтерации.ТекущийШагДозапускаПользователей;
		КонецЕсли;
		
	КонецЕсли;
	
	Если НужныДополнительныеИтерации Тогда
		ДополнитьТаблицуВыполнения(ПараметрыИтерации.НомерТекущейИтерации + 1, ТаблицаВыполнения, Сценарий, Тест, ПараметрыВыполненияСценария);
	Иначе
		
		Если ПараметрыВыполненияСценария.ЭтоДинамическийТест Тогда
			ДобавитьЗаданияЗаписиИУдаления(ПараметрыИтерации.НомерТекущейИтерации, ТаблицаВыполнения, Сценарий, Тест);
		КонецЕсли;
		
	КонецЕсли;
	
	Если ПараметрыВыполненияСценария.ЭтоДинамическийТест Тогда
		ПараметрыИтерации.НачалоТекущегоСостоянияВыполнения = ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения + 1;
	Иначе
		ПараметрыИтерации.НачалоТекущегоСостоянияВыполнения = Неопределено;
	КонецЕсли;
	
	ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения = Неопределено;	
	
КонецПроцедуры

// Процедура - Удалить не ответивших агентов
//
// Параметры:
//  НеОтветившиеАгенты				 - 	 Массив
//  КоличествоАгентовПоКомпьютерам	 - 	 Соответствие
//  ТаблицаРаспределения			 - 	 ТаблицаЗначений
//  ПредыдущийСценарийЗапуска		 - 	 ТаблицаЗначений или Неопределено
//
Процедура УдалитьНеОтветившихАгентов(Агенты, НеОтветившиеАгенты, КоличествоАгентовПоКомпьютерам, ТаблицаРаспределения, ПредыдущийСценарийЗапуска)
	...
	Если ПредыдущийСценарийЗапуска <> Неопределено Тогда
		...
		Пока Сч >= 0 Цикл
			...
			Если НеОтветившиеАгенты.Найти(ТекущийАгент) <> Неопределено Тогда
	...
	Для Каждого ТекущийНеОтветивший Из НеОтветившиеАгенты Цикл
		...
		ИндексАгента = Агенты.Найти(ТекущийНеОтветивший);
		Если ИндексАгента <> Неопределено Тогда
			Агенты.Удалить(ИндексАгента);
		КонецЕсли;
		
		ТекущееКоличествоАгентов = КоличествоАгентовПоКомпьютерам[ТекущийНеОтветивший.Компьютер];
		Если ТекущееКоличествоАгентов <> Неопределено Тогда
			КоличествоАгентовПоКомпьютерам.Вставить(ТекущийНеответивший.Компьютер, ТекущееКоличествоАгентов - 1);
		КонецЕсли;
	КонецЦикла;
	
	АгентыКЗавершению = ТЦСервер.КлиентыЗапущены(НеОтветившиеАгенты);
	...
	Если ТаблицаРаспределения <> Неопределено Тогда
		Для Каждого ТекущийЭлемент Из КоличествоАгентовПоКомпьютерам Цикл
...
//  Соответствие
//
Функция ПолучитьКоличествоВРМПоКомпьютерам(СтруктураСценария)
	...
	КоличествоВРМПоКомпьютерам = Новый Соответствие;
	Для Каждого ТекущаяСтрока Из СценарийКопия Цикл
		КоличествоВРМПоКомпьютерам.Вставить(ТекущаяСтрока.Компьютер, ТекущаяСтрока.Количество);
	КонецЦикла;
	
	Возврат КоличествоВРМПоКомпьютерам;
...
//
Функция ПолучитьКоличествоВРМАгентов(Агенты)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ТЦСценарийЗапуска.Агент,
	               |	КОЛИЧЕСТВО(*) КАК КоличествоВРМАгента
	               |ИЗ
	               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска
	               |ГДЕ
	...
	КоличествоВРМАгентов = Новый Соответствие;
	Для Каждого ТекАгент Из Агенты Цикл
		
		КоличествоВРМАгента = 0;
		СтуктураПоиска = Новый Структура("Агент");
		СтуктураПоиска.Агент = ТекАгент;
		Если Выборка.НайтиСледующий(СтуктураПоиска) Тогда
			КоличествоВРМАгента = Выборка.КоличествоВРМАгента;
			Выборка.Сбросить();
		КонецЕсли;
		
		КоличествоВРМАгентов.Вставить(ТекАгент, КоличествоВРМАгента);

	КонецЦикла;
	
	Возврат КоличествоВРМАгентов;
...
//
Функция СформироватьТаблицуВыполнения(Сценарий, Тест, ПараметрыВыполненияСценария)
	...
	Параметры.Вставить("ЭтоДинамическийТест",		ПараметрыВыполненияСценария.ЭтоДинамическийТест);
	Параметры.Вставить("КоэффициентИнтенсивности",	ПараметрыВыполненияСценария.КоэффициентИнтенсивности);
	Параметры.Вставить("ОбщееКоличествоОдновременноЗапускаемыхВРМ",	ПараметрыВыполненияСценария.ОбщееКоличествоОдновременноЗапускаемыхВРМ);
	Параметры.Вставить("КоличествоОдновременноЗапускаемыхВРМАгента",	ПараметрыВыполненияСценария.КоличествоОдновременноЗапускаемыхВРМАгента);
	...
	Задание = Таблица.Добавить();
	Параметры = Новый Структура;
	Параметры.Вставить("Состояние", Перечисления.ТЦСостояние.Загрузка);
	Задание.Параметры = Параметры;
	Задание.Таймаут = Тест.ТаймаутПодготовки;
	Задание.Ошибка = "ТаймаутЗагрузки";
	Задание.НомерИтерации = 1;
	Задание.ЭтоПовтор = Ложь;
	...
	Если Не ПараметрыВыполненияСценария.ЭтоДинамическийТест Тогда
		Задание = Таблица.Добавить();
		Параметры = Новый Структура;
...
// Функция - Получить параметры подготовки
//
// Параметры:
//  Тест						 - ДокументСсылка.ТЦТест
//  Сценарий					 - СправочникСсылка.ТЦСценарии
//  НомерИтерации				 - Число
//  ПараметрыВыполненияСценария	 - Структура
//  Состояние					 - ПеречислениеСсылка.ТЦСостояние
// 
// Возвращаемое значение:
//  Структура
//
Функция ПолучитьПараметрыПодготовки(Тест, Сценарий, НомерИтерации, ПараметрыВыполненияСценария, Состояние)
...
Процедура ДобавитьПовторнуюПодготовку(НомерИтерации, ИндексЗадания, ТаблицаВыполнения, Сценарий, Тест, ПараметрыВыполненияСценария)
	
	Задание = ТаблицаВыполнения.Вставить(ИндексЗадания);
	Параметры = ПолучитьПараметрыПодготовки(Тест, Сценарий, НомерИтерации, ПараметрыВыполненияСценария, Перечисления.ТЦСостояние.Подготовка);
...
// Добавить задание повторной подготовки в таблицу выполнения
Процедура ДобавитьПовторнуюПодготовкуИЗагрузку(НомерИтерации, ИндексЗадания, ТаблицаВыполнения, Сценарий, Тест, ПараметрыВыполненияСценария)
	...
	Задание = ТаблицаВыполнения.Вставить(ИндексЗадания + 1);
	Параметры = Новый Структура;
	Параметры.Вставить("Состояние", Перечисления.ТЦСостояние.Загрузка);
	Задание.Параметры = Параметры;
	Задание.Таймаут = Тест.ТаймаутПодготовки;
...
//  ЭтоДинамическийТест - Булево
//
Процедура ДополнитьТаблицуВыполнения(НомерИтерации, ТаблицаВыполнения, Сценарий, Тест, ПараметрыВыполненияСценария)
...
Функция ПолучитьМасштабированнуюСтруктуруСценария(Сценарий, СценарийПредыдущейИтерации,
								...
								КоличествоПользователейПредыдущейИтерации = 0,
								АдресПубликации,
								ЕстьРазделение,
	...
	Если ТаблицаРаспределения <> Неопределено Тогда
		...
		Если ТаблицаРаспределения.Колонки.Найти("Количество") <> Неопределено Тогда
			ТаблицаРаспределения.Свернуть("Компьютер", "Вес,Количество");
		Иначе
	...
	Если ТаблицаРаспределения <> Неопределено Тогда
		...
		ТаблицаРаспределения.Свернуть("Компьютер", "Вес, Количество");
		ТаблицаРаспределения.Колонки.Добавить("КоличествоКЗапуску", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(10,0)));
		
		ТаблицаРаспределения.Сортировать("Вес Возр, Количество Возр, Компьютер");
		
		ВесОстаток = ТаблицаРаспределения.Итог("Вес");
		
		НельзяУменьшатьКоличествоВРМ = (ЦелевоеКоличествоПользователей >= КоличествоПользователейПредыдущейИтерации);
		ЦелевоеКоличествоПользователейОстаток = ЦелевоеКоличествоПользователей;
		ПользователейРаспределено = ТаблицаРаспределения.Итог("Количество");
		ОсталосьРаспределить = ЦелевоеКоличествоПользователей - ПользователейРаспределено;
		
		Для Каждого ТекСтрока Из ТаблицаРаспределения Цикл
			
			ТекПланируемоеКоличество = Окр(ТекСтрока.Вес * ЦелевоеКоличествоПользователейОстаток / ВесОстаток);
			Дельта = ТекПланируемоеКоличество - ТекСтрока.Количество;
			
			// Если очередная итерация теста должна выполнится на количестве ВРМ равным, либо превышаем количество ВРМ предыдущей итерации,
			// необходимо учесть, что нельзя уменьшать количество ВРМ на компьютерах.
			// (Без условия ниже новое расчетное количество может измениться в меньшую сторону,
			// если на некоторых компьютерах или агентах происходили аварии).
			Если Дельта <> 0 
				И НЕ (НельзяУменьшатьКоличествоВРМ И ТекПланируемоеКоличество < ТекСтрока.Количество)  Тогда
				
				// Изменяем остаток распределения
				ТекСтрока.Количество = ТекПланируемоеКоличество;
				ОсталосьРаспределить = ОсталосьРаспределить - Дельта;
				
			КонецЕсли;
					
			ВесОстаток = ВесОстаток - ТекСтрока.Вес;
			ЦелевоеКоличествоПользователейОстаток = ЦелевоеКоличествоПользователейОстаток - ТекСтрока.Количество;
			ТекСтрока.КоличествоКЗапуску = ТекСтрока.Количество;
	...
	Если УжеБылаИтерация Тогда
		...
		Если СценарийПредыдущейИтерации.Колонки.Найти("Компьютер") = Неопределено Тогда
			СценарийПредыдущейИтерации.Колонки.Добавить("Компьютер", Новый ОписаниеТипов("СправочникСсылка.ТЦКомпьютеры"));
			
			Для Каждого ТекСтрока Из СценарийПредыдущейИтерации Цикл
				ТекСтрока.Компьютер = ТекСтрока.Клиент.Компьютер;
			КонецЦикла;
			
		КонецЕсли;

		// Если на одном из компьютеров уже нет агентов, то удалим строки
		// сценария предыдущей итерации с данным компьютером.
		Если ТаблицаРаспределения <> Неопределено Тогда
		...
		Если ТаблицаРаспределения <> Неопределено Тогда
			...
			Для Каждого СтрокаВеса из ТаблицаРаспределения Цикл
				...
				Если СтрокаКоличестваСтарыхВРМ <> Неопределено Тогда
					СтрокаВеса.КоличествоКЗапуску = СтрокаВеса.КоличествоКЗапуску - СтрокаКоличестваСтарыхВРМ.Количество;
				КонецЕсли;
	...
	Если ТаблицаРаспределения <> Неопределено Тогда
		...
		Для Каждого СтрокаВеса Из ТаблицаРаспределения Цикл
			...
			Для НомерВРМ = 1 По СтрокаВеса.КоличествоКЗапуску Цикл
...
Процедура ОтметитьВРМКЗапуску(ОбщееКоличествоОдновременноЗапускаемыхВРМ) 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ТЦСценарийЗапуска.СостояниеПодготовки = ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.ГотовКЗапуску)
	               |					ТОГДА 1
	...
	Если ВРМКОтметке > 0 Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ " + Формат(ВРМКОтметке, "ЧГ=0") + " 
		               ...
		               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска
		               |ГДЕ
		               |	ТЦСценарийЗапуска.СостояниеПодготовки = ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.НеГотовКЗапуску)";
		
		Выборка = Запрос.Выполнить().Выбрать();

		Пока Выборка.Следующий() Цикл
			ОтметитьСостояниеПодготовкиВРМ(Выборка.Ид, Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗапуску);
		КонецЦикла;
...
// Отмечает указанное количество ВРМ как готовые к загрузке
//
// Параметры:
//  ОбщееКоличествоОдновременноЗапускаемыхВРМ  - Число
//
Процедура ОтметитьВРМКЗагрузке(ОбщееКоличествоОдновременноЗапускаемыхВРМ) 
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ТЦСценарийЗапуска.СостояниеПодготовки = ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.ГотовКЗагрузке)
	               |					ТОГДА 1
	               |			КОНЕЦ), 0) КАК КоличествоГотовых,
	               |	ЕСТЬNULL(СУММА(ВЫБОР
	               |				КОГДА ТЦСценарийЗапуска.СостояниеПодготовки = ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.НеГотовКЗагрузке)
	               |					ТОГДА 1
	               |			КОНЕЦ), 0) КАК КоличествоНеготовых
	               |ИЗ
	               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска";
	...
	Если ВРМКОтметке > 0 Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ ПЕРВЫЕ " + Формат(ВРМКОтметке, "ЧГ=0") + "
		               ...
		               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска
		               |ГДЕ
		               |	ТЦСценарийЗапуска.СостояниеПодготовки = ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.НеГотовКЗагрузке)
		               |
		               |УПОРЯДОЧИТЬ ПО
		...
		Пока Выборка.Следующий() Цикл
			ОтметитьСостояниеПодготовкиВРМ(Выборка.Ид, Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗагрузке);
		КонецЦикла;
		
	КонецЕсли;
		
КонецПроцедуры // ОтметитьВРМКЗагрузке()
...
Процедура ПодготовитьСценарийЗапуска(СтруктураСценария, ИспользоватьТолькоКлонов, ПредыдущийСценарийЗапуска, ЭтоДинамическийТест = Ложь, ОбщееКоличествоОдновременноЗапускаемыхВРМ = 0) Экспорт
	...
	// Получить сценарий
	СтруктураСценария.Свернуть("Пользователь,Клиент,Роль", "Количество");
	...
	Для Каждого СтрокаСценария Из СтруктураСценария Цикл
		...
		Для Сч = 1 По СтрокаСценария.Количество Цикл
			...
			Если ОбщееКоличествоОдновременноЗапускаемыхВРМ = 0 Тогда
				ЭлементЗапуска.СостояниеПодготовки = Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗапуску;
			Иначе
				ЭлементЗапуска.СостояниеПодготовки = Перечисления.ТЦСостояниеПодготовкиВРМ.НеГотовКЗапуску;
			КонецЕсли;
...
Функция ПолучитьУдовлетворительностьПроизводительности(ПараметрыВыполненияСценария, ПараметрыИтерации)
	
	Если НЕ ОценкаПроизводительностиИспользуется() Тогда
		ПараметрыИтерации.ТекущаяПроизводительность = 0;
		ПараметрыИтерации.ТекущееКоличествоКО = 0;
		Возврат Истина;		
	КонецЕсли;
	
	Если ПараметрыВыполненияСценария.ПороговыйAPDEX > 1 ИЛИ ПараметрыВыполненияСценария.ПороговыйAPDEX < 0 Тогда
		ПараметрыИтерации.ТекущаяПроизводительность = 0;
		ПараметрыИтерации.ТекущееКоличествоКО = 0;
		Возврат Ложь;
	КонецЕсли;
	...
	Запрос.УстановитьПараметр("ДатаНачалаЗамеров", ПараметрыИтерации.НачалоТекущегоСостоянияВыполнения);
	Запрос.УстановитьПараметр("ДатаОкончанияЗамеров", ПараметрыИтерации.ОкончаниеТекущегоСостоянияВыполнения);
	...
	ПараметрыИтерации.ТекущаяПроизводительность = Выборка.APDEX;
	ПараметрыИтерации.ТекущееКоличествоКО = Выборка.КоличествоКО;
	
	Если Выборка.APDEX < ПараметрыВыполненияСценария.ПороговыйAPDEX Тогда
		Возврат Ложь;
	КонецЕсли;
...
Функция ВРМВнеСценария(Компьютер, НомерСессии) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	СценарийЗапуска.СостояниеПодготовки ЕСТЬ NULL ";
...
//  ОбщееКоличествоОдновременноЗапускаемыхВРМ - число
//
Функция ПереиспользоватьВРМ(Компьютер, НомерСессии, ПродолжающиеРаботуВРМ, ОбщееКоличествоОдновременноЗапускаемыхВРМ) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |			И ВП.Клон = СценарийЗапуска.Клон
	               |ГДЕ
	               |	СценарийЗапуска.СостояниеПодготовки ЕСТЬ НЕ NULL 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрокиСценарияЗапуска.Количество() > 0 Тогда
			...
			Если НайденВПродолжающихРаботу Тогда
				СтрокиСценарияЗапуска[0].СостояниеПодготовки = Перечисления.ТЦСостояниеПодготовкиВРМ.Загружен;
			ИначеЕсли ОбщееКоличествоОдновременноЗапускаемыхВРМ > 0 Тогда
				СтрокиСценарияЗапуска[0].СостояниеПодготовки = Перечисления.ТЦСостояниеПодготовкиВРМ.НеГотовКЗагрузке;
			Иначе
				СтрокиСценарияЗапуска[0].СостояниеПодготовки = Перечисления.ТЦСостояниеПодготовкиВРМ.ГотовКЗагрузке;
			КонецЕсли;
...
Функция ТаймаутПодготовки(Сценарий, Тест)
	...
	Возврат "Следующие агенты не отчиталась о запуске ВРМ на этапе подготовки (таймаут подготовки "+ Тест.ТаймаутПодготовки + " с):";
	
КонецФункции // ТаймаутПодготовки()

// Получить описание таймаута загрузки
//
// Параметры:
//  Сценарий - СправочникСсылка.ТЦСценарии
//  Тест - ДокументСсылка.ТЦТест
...
//  Строка - описание
//
Функция ТаймаутЗагрузки(Сценарий, Тест)
	
	Возврат "Следующие агенты не отчиталась о загрузке ВРМ на этапе подготовки (таймаут подготовки "+ Тест.ТаймаутПодготовки + " с):";
	
КонецФункции // ТаймаутЗагрузки()
...
Функция ТаймаутИнициализации(Сценарий, Тест)
	...
	Возврат "Следующие агенты не ответили за таймаут инициализаци ВРМ ("+ Тест.ТаймаутИнициализации + " с):";
...
Функция ТаймаутВыполнения(Сценарий, Тест)
	...
	Возврат "Следующие агенты не ответили за таймаут выполнения сценария ("+ Тест.ТаймаутВыполнения + " с):";
...
Функция ТаймаутЗаписиРезультатов(Сценарий, Тест)
	...
	Возврат "Следующие агенты не ответили за таймаут записи результатов ("+ Тест.ТаймаутЗаписиРезультатов + " с):";
...
Функция ТаймаутУдаленияДанных(Сценарий, Тест)
	...
	Возврат "Следующие агенты не ответили за таймаут удаления данных ("+ Тест.ТаймаутУдаленияДанных + " с):";
...
Функция ТестПрерван(Тест)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не удалось найти тест при проверке прерывания.'");
	КонецЕсли;
...
Функция ТестЗавершен(Тест)
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Не удалось найти тест при проверке завершения.'");
	КонецЕсли;
...
Функция ПолучитьСообщение(Адрес, От = Неопределено, Удалить = Истина) Экспорт
	...
	Исключение
		...
		ТекстОшибки = "Не удалось получить сообщение (ТЦОчередьСообщений): " + ТЦОбщий.ИнформациюОбОшибкеВСтроку(ИнформацияОбОшибке());
...
// Активировать ВРМ по состояниям ВРМ
//
// Параметры:
//  СостоянияВРМ — соответствие
//
Процедура АктивироватьВРМПоСостояниям(СостоянияВРМ) Экспорт
	
	НачатьТранзакцию();
	
	Для Каждого СостояниеВРМ Из СостоянияВРМ Цикл
		
		Если СостояниеВРМ.Значение.Выполнено Тогда
...
КонецПроцедуры // АктивироватьВРМПоСостояниям()

// Процедура - Отметить ВРМЗагружен по состояниям
//
// Параметры:
//  СостоянияВРМ - Соотвествие
//
Процедура ОтметитьВРМЗагруженПоСостояниям(СостоянияВРМ) Экспорт
	...
	Для Каждого СостояниеВРМ Из СостоянияВРМ Цикл
		...
		Если СостояниеВРМ.Значение.Выполнено Тогда
			МассивВРМ.Добавить(СостояниеВРМ.Ключ);
		КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ТЦВП КАК ТЦВП
	               |		ПО (ТЦВП.Клон = ТЦСценарийЗапуска.Клон)
	               |			И (ТЦВП.Ссылка В (&МассивВРМ))
	               |			И (ТЦВП.Состояние <> ЗНАЧЕНИЕ(Перечисление.ТЦСостояние.Выгрузка))";
	
	Запрос.УстановитьПараметр("МассивВРМ", МассивВРМ);
	...
	Пока Выборка.Следующий() Цикл
		ОтметитьСостояниеПодготовкиВРМ(Выборка.Ид, Перечисления.ТЦСостояниеПодготовкиВРМ.Загружен);
	КонецЦикла;	
...
//
// Параметры:
//		СписокВРМ — список ссылок на ВРМ
//
//
Процедура ЗагрузитьВРМ(СписокВРМ) Экспорт

	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СписокВРМ", СписокВРМ);
	...
	Попытка
		...
		Пока Выборка.Следующий() Цикл
			...
			ОбъектВРМ.Состояние = Перечисления.ТЦСостояние.Загрузка;
...
Функция НеЗапущенныеВРМ(ИмяКомпьютера, НомерСессии, Количество) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И ТЦСценарийЗапуска.СостояниеПодготовки = ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.НеГотовКЗапуску)
				   |;
				   |////////////////////////////////////////////////////////////////////////////////
				   |
...
// Получить первые N незагруженных ВРМ, готовых к загрузке
//
// Параметры:
//  ИмяКомпьютера - Строка, имя компьютера на котором будут запускаться ВРМ
//  НомерСессии - Число, номер сессии в которой будут запускаться ВРМ
//  Количество - Число, количество загружаемых ВРМ
// Возвращаемое значение:
//  Массив - ВРМ, которые необходимо запустить
//
Функция НеЗагруженныеВРМ(ИмяКомпьютера, НомерСессии, Количество) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               |	КОЛИЧЕСТВО(ТЦСценарийЗапуска.Ид) КАК КоличествоНеготовыхКЗагрузке
	               |ПОМЕСТИТЬ НеготовыеКЗагрузке
	               |ИЗ
	               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ТЦВП КАК ТЦВП
	               |		ПО (ТЦВП.Клон = ТЦСценарийЗапуска.Клон)
	               |			И (ТЦВП.Состояние = ЗНАЧЕНИЕ(Перечисление.ТЦСостояние.Подготовка)
	               |				ИЛИ ТЦВП.Состояние = ЗНАЧЕНИЕ(Перечисление.ТЦСостояние.Неактивен))
	               |ГДЕ
	               |	ТЦСценарийЗапуска.Компьютер = &Компьютер
	               |	И ТЦСценарийЗапуска.НомерСессии = &НомерСессии
	               |	И ТЦСценарийЗапуска.СостояниеПодготовки = ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.НеГотовКЗагрузке)
				   |;
	               |////////////////////////////////////////////////////////////////////////////////
				   |
				   |ВЫБРАТЬ ПЕРВЫЕ " + Формат(Количество, "ЧГ=0") + "
	               |	ЕСТЬNULL(ТЦВП.Ссылка, """") КАК Ссылка,
	               |	ПРЕДСТАВЛЕНИЕ(ТЦСценарийЗапуска.Роль) КАК Роль,
	               |	ПРЕДСТАВЛЕНИЕ(ТЦСценарийЗапуска.Клон) КАК Клон,
	               |	ПРЕДСТАВЛЕНИЕ(ТЦСценарийЗапуска.Компьютер) КАК Компьютер,
	               |	ПРЕДСТАВЛЕНИЕ(ТЦСценарийЗапуска.НомерСессии) КАК НомерСессии,
	               |	ТЦСценарийЗапуска.Ид КАК Ид
	               |ПОМЕСТИТЬ ГотовыеКЗагрузке
	               |ИЗ
	               |	РегистрСведений.ТЦСценарийЗапуска КАК ТЦСценарийЗапуска
	               ...
	               |	ТЦСценарийЗапуска.Компьютер = &Компьютер
	               |	И ТЦСценарийЗапуска.НомерСессии = &НомерСессии
	               |	И ТЦСценарийЗапуска.СостояниеПодготовки = ЗНАЧЕНИЕ(Перечисление.ТЦСостояниеПодготовкиВРМ.ГотовКЗагрузке)
	               |";
	...
	ГотовыеКЗагрузке = Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("ГотовыеКЗагрузке");
	
	ВозвращаемыйРезультат = Новый Структура;
	ВозвращаемыйРезультат.Вставить("ГотовыеКЗагрузке", ТаблицуЗначенийВМассивСтруктур(ГотовыеКЗагрузке.ПолучитьДанные().Выгрузить()));
	
	НеготовыеКЗагрузке = Запрос.МенеджерВременныхТаблиц.Таблицы.Найти("НеготовыеКЗагрузке");
	Выборка = НеготовыеКЗагрузке.ПолучитьДанные().Выбрать();
	Выборка.Следующий();
	
	ВозвращаемыйРезультат.Вставить("КоличествоНеготовыхКЗагрузке", Выборка.КоличествоНеготовыхКЗагрузке);
	
	Возврат ВозвращаемыйРезультат;
	
КонецФункции // НеЗагруженныеВРМ()
...
//
Процедура ОтметитьЗапускВыполнен(Запуск, ОбщееКоличествоОдновременноЗапускаемыхВРМ) Экспорт
...
// Устатановить состояние подготовки ВРМ
//
// Параметры:
//  ИдЗапуска - значение измерения Ид из регистра сведений ТЦСценарийЗапуска
//  СостояниеПодготовки — ПеречислениеСсылка.ТЦСостояниеПодготовкиВРМ — устанавливаемое состояние
//
Процедура ОтметитьСостояниеПодготовкиВРМ(ИдЗапуска, СостояниеПодготовки)
	
	СтрокиСценарияЗапуска = РегистрыСведений.ТЦСценарийЗапуска.СоздатьНаборЗаписей();
	СтрокиСценарияЗапуска.Отбор.Ид.Установить(ИдЗапуска);
	СтрокиСценарияЗапуска.Прочитать();
	
	Если СтрокиСценарияЗапуска.Количество() > 0 Тогда
		
		СтрокиСценарияЗапуска[0].СостояниеПодготовки = СостояниеПодготовки;
		СтрокиСценарияЗапуска.ОбменДанными.Загрузка = Истина;
		СтрокиСценарияЗапуска.Записать();
...
// Отметить ошибки загрузки ВРМ в сценарии запуска
//
// Параметры:
//  МассивНезагруженных - массив структур, содержащих поле "Ид" для регистра Сценарий запуска
//
Процедура ОтметитьОшибкиЗагрузкиВСценарииЗапуска(МассивНезагруженных) Экспорт
	
	Для Каждого ТекЭлемент Из МассивНезагруженных Цикл
		ОтметитьСостояниеПодготовкиВРМ(ТекЭлемент.Ид, Перечисления.ТЦСостояниеПодготовкиВРМ.ОшибкаЗагрузки);
	КонецЦикла;
...
Процедура СоздатьВРМ(Наименование, Клиент, ИмяКомпьютера, НомерСессии, Пользователь, Клон, Порт, Ссылка) Экспорт
	...
	// По умолчанию ВРМ создается в состоянии подготовки.
	// После этапа подготовки ВРМ должно переводиться агентов в состояние загрузки.
	ЭтотОбъект.Состояние = Перечисления.ТЦСостояние.Подготовка;
	ЭтотОбъект.ЭтапЗавершен = Истина;
...
Процедура СинхронизироватьПользователей(Пользователь, Оригинал, НовоеИмя)
	...
	Если Пользователь.ЗащитаОтОпасныхДействий <> Оригинал.ЗащитаОтОпасныхДействий  Тогда
		Пользователь.ЗащитаОтОпасныхДействий  = Оригинал.ЗащитаОтОпасныхДействий;
		Изменен = Истина;
	КонецЕсли;
...
Функция БСПИспользуетсяСправочникИдентификаторовОбъектовМетаданных() Экспорт
	...
	Если Метаданные.Справочники.Найти("ИдентификаторыОбъектовМетаданных") <> Неопределено Тогда
		...
		ОписаниеТиповРоли = Метаданные.Справочники.ПрофилиГруппДоступа.ТабличныеЧасти.Роли.Реквизиты.Роль.Тип;
		
		Если ОписаниеТиповРоли.СодержитТип(Тип("СправочникСсылка.ИдентификаторыОбъектовМетаданных")) Тогда
			Возврат Истина;
		КонецЕсли;
...
Функция НомерСостояния(Состояние) Экспорт
	...
	Если Состояние = Перечисления.ТЦСостояние.Подготовка ИЛИ Состояние = Перечисления.ТЦСостояние.Загрузка Тогда
		Возврат 1;
...
//
Процедура СформироватьОтчетПоПротоколу(Начало, Окончание, Док, ДополнительныйОтчет = Неопределено) Экспорт
	...
	СформироватьОтчет(Схема, Док,, ОписаниеЖурнала, ДополнительныйОтчет);
...
//  
//
Процедура СформироватьОтчет(Схема, Док, Параметры = Неопределено, Данные = Неопределено, ДополнительныйДокумент = Неопределено) Экспорт
	...
	Если ДополнительныйДокумент <> Неопределено Тогда
		...
		ПроцессорВывода.УстановитьДокумент(ДополнительныйДокумент);
		ПроцессорВывода.Вывести(ПроцессорКомпоновки);
	КонецЕсли;
...
//
Процедура СформироватьОтчетПоПроизводительности(Тест, ТабличныйДокумент, ДополнительныйДокумент = Неопределено) Экспорт

	СхемаКомпоновкиДанных = Отчеты.ТЦАнализЗамеров.ПолучитьМакет("ОсновнаяСхемаКомпоновкиДанных");
	СформироватьОтчет(СхемаКомпоновкиДанных, ТабличныйДокумент, Новый Структура("Тест", Тест),,ДополнительныйДокумент);
...
//  ТабличныйДокумент  - ТабличныйДокумент
//
Процедура СформироватьОтчетПоОшибкам(Тест, ТабличныйДокумент, ДополнительныйДокумент = Неопределено) Экспорт
...
Процедура ПроцедурыИФукнцииИспользуемыеНеявно()
	...
	ПолучитьТипПриложенияИзПараметровСеанса();
	ЗавершитьТест(Неопределено);
	ПрерватьТест(Неопределено);
	ТаймаутЗагрузки(Неопределено, Неопределено);
	ТаймаутВыполнения(Неопределено, Неопределено);
	ТаймаутЗаписиРезультатов(Неопределено, Неопределено);
	ТаймаутИнициализации(Неопределено, Неопределено);
	ТаймаутПодготовки(Неопределено, Неопределено);
	ТаймаутУдаленияДанных(Неопределено, Неопределено);
	ТестВыполненСОшибкой(Неопределено, Неопределено);
	ТестВыполненУспешно(Неопределено);
	ТестЗавершен(Неопределено);
	УстановитьМенеджерТеста(Неопределено, Неопределено);
	УстановитьПаузуПослеЗапускаВРМ(Неопределено, Неопределено);
	УстановитьСостояниеЗапускаВРМ(Неопределено, Неопределено);
	УстановитьСостояниеТеста(Неопределено, Неопределено);
	УстановитьСостояниеТестаПовторно(Неопределено, Неопределено);
	ПроцедурыИФукнцииИспользуемыеНеявно();
	БСПНайтиПользователияПоИмени(Неопределено);
